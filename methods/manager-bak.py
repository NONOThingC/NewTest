    ...
    def train(self, args):
        ...
        for i in range(args.total_round):
            ...
            for steps, (training_data, valid_data, test_data, current_relations, historic_test_data, seen_relations) in enumerate(sampler):
                ...
                #
                # all_current_embeddings,indss,cur_labelss=self.get_embedding( args, encoder, train_data_for_initial)
                _, all_current_embeddings, _ = self.select_data(args, encoder,  train_data_for_initial) # K*hidden
                # repaly
                if len(memorized_samples)>0:
                    # select current task sample
                    # self.update_all_embedding(args, encoder,retrieval_pool, all_data_for_pool)
                    
                    # retrieve old data using prototype, old data and embedding.
                    
                    all_embeddings,indss,labelss=self.get_embedding( args, encoder, list(itertools.chain(*all_data_for_pool.values())))
                    retrieval_pool.reset_index()
                    retrieval_pool.add_to_retrieve_pool(all_embeddings,ids=indss,labels=labelss)
                    
                    
                    
                    ret_vec,indexes=retrieval_pool.retrieval_old_data(q=all_current_embeddings) ## 用老的prototype去retrieve错误
                    ret_vec= torch.tensor(ret_vec,device=args.device)
                    train_data_for_memory += indexes
                    # 给一些旧数据用于训练，给一些prototype用于蒸馏
                    
                    for relation in current_relations:
                        retrieval_pool.update_all_old()
                        memorized_samples[relation], _, _ = self.select_data(args, encoder, training_data[relation])
                        
                    proto4repaly=self.calib_proto(old_proto,ret_vec) ## 修正这些错误
                    ...
                    # self.add_new_data( args, encoder, train_data_for_initial)
                    # all_current_embeddings,indss,labelss=self.get_embedding( args, encoder, train_data_for_initial)
                    # retrieval_pool.add_to_retrieve_pool(all_current_embeddings,indss)
                # else:
                #     #之前类别数据添加到池子里
                #     retrieval_pool.add_to_retrieve_pool(all_current_embeddings,indss)
                ...
                for relation in history_relation:
                    if relation not in current_relations:# old relation
                        ...
                        protos, _ = self.get_proto(args, encoder, memorized_samples[relation])## TODO:new get old proto by calibration
                ...
                print(test_cur)
                print(test_total)
                all_data_for_pool=all_data_for_pool.update(training_data) # for retreval pool update
                del self.moment
